...
module BaUpload
  def self.export_certificate(file:, passphrase:)
    cert = OpenSSL::PKCS12.new(file, passphrase)
    {
  ...
  def self.open_connection(file:, passphrase:)
    cert = BaUpload.export_certificate(file: file, passphrase: passphrase)
